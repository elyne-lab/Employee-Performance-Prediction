...
def main():
    ...
    # Top 15 important features (update these based on your model's feature importance)
    top_features = {
    ...
    if page == "Prediction":
        ...
        st.sidebar.info(
            """
            This prediction is based on the top 15 most influential features 
    ...
    unsafe_allow_html=True,
        ...
        Accounting for over 50 percent of the employee Performance rate behavior.
                 
        1. EmpLastSalaryHikePercentage (21%)
        2. EmpEnvironmentSatisfaction (22%)
        3. YearsSinceLastPromotion (9%)

        Recent salary increases and workplace environment strongly contribute to the employee performance 
                 with the frequency of promotions following closely.

        ### Key Findings
        INX Future Inc company has a generally good work culture as seen in good average scores for employee
                 environment satisfaction and job satisfaction coupled with even better scores for employee work life balance.

        There is also potential for professional growth shown by the short tenure in current roles (0-5) years
                 and quick promotion cycles less than two years.

        INX Future Inc might rank best in retention of talent.
